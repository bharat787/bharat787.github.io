...
export default function App() {
  return <>
    ...
    <header><a className="wordmark" href="#introduction" aria-label="Bharat Gupta home">bg<span>.</span></a><nav aria-label="Main navigation"><a href="#work">Work & projects</a><a href="#socials">Let’s connect <span>↗</span></a></nav></header>
    <TransitionScene />
    <main>
      <section className="intro scene" id="introduction">
        <div className="intro-copy"><p className="eyebrow">BHARAT GUPTA / A WORK IN PROGRESS</p><h1>Hi, I’m Bharat <span className="greeting">ヾ(＾-＾)ノ</span></h1>
          {content.introduction.map((paragraph, i) => <p key={i} className={i === 0 ? 'lead' : undefined} dangerouslySetInnerHTML={{ __html: paragraph }} />)}
          <a className="scroll-cue" href="#work">Scroll to explore <span>↓</span></a>
        </div>
      </section>
      <section className="work scene" id="work" aria-label="Work experience and projects">
        ...
        <div className="projects-column" id="projects"><p className="eyebrow">02 / WHAT I’VE BUILT</p><h2>Projects<span>.</span></h2>
          {content.projects.map((project, i) => <article className="project" key={project.title}><span className="project-number">{String(i + 1).padStart(2, '0')}</span><div><h3>{project.url ? <a href={project.url} target="_blank" rel="noreferrer">{project.title} <span className="arrow">↗</span></a> : project.title}</h3>{project.description && <p>{project.description}</p>}{!project.url && <span className="status">Beta soon</span>}</div></article>)}
